...
<form action="car_del.action" method="post">
    <table width="85%" cellspacing="0" align="center">
        ...
        <tr align="center" height="30" onMouseOver="this.bgColor='#F8EFF2'" onMouseOut="this.bgColor='#ffffff'">
            <td width="8%">
                <input type="checkbox" name="aa" value="<%=car.getId()%>"/>
            </td>
            <td width="8%"><%=car.getId()%></td>
            <td width="10%"><%=car.getPingpai()%>
            </td>
            <td width="15%"><%
            ...
            <td width="12%"><%=car.getPeople() %>
            </td>
            <td width="10%"><%=car.getCount() %>
            </td>
            <td width="10%"><%=car.getCarid()%>
            </td>
            <td width="9%"><%=car.getZhuangtai() %>
            </td>
            <td width="18%">
                <a href="car_id.action?id=<%=car.getId()%>">[修改]</a>
        ...
        <tr style="font-size:12px; height:50px;text-align:center;">
            <td colspan="10">共&nbsp;<font size="2px" color=red><%=recordCount %>
                ...
                <a href="car.action?page=1">首页</a>&nbsp;&nbsp;
                <a href="car.action?page=<%=currentPage-1%>">上一页</a>&nbsp;&nbsp;
                <%
                    for (int j = 1; j <= maxPage; j++) {
                        out.print("&nbsp;&nbsp;<a href='car.action?page=" + j + "'><span class=STYLE20>" + j + "</span></a>&nbsp;&nbsp;");
                    }
                %>
                <a href="car.action?page=<%=currentPage+1%>">下一页</a>&nbsp;&nbsp;
                <a href="car.action?page=<%=maxPage%>">最后一页</a></td>
        </tr>
    </table>
</form>
</body>
